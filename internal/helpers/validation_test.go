...
func TestValidateClientName(t *testing.T) {
	tests := []struct {
		...
		{
			name:       "rejects single quote (JS injection)",
			clientName: "App's Name",
			wantErr:    true,
			errContain: "special characters",
		},
		{
			name:       "rejects double quote (JS injection)",
			clientName: `App "Special" Name`,
			wantErr:    true,
			errContain: "special characters",
		},
		{
			name:       "rejects backtick (template literal injection)",
			clientName: "App `template` Name",
			wantErr:    true,
			errContain: "special characters",
		},
		{
			name:       "rejects JS template literal injection",
			clientName: "`${alert(1)}`",
			wantErr:    true,
			errContain: "special characters",
		},
		{
			name:       "rejects mixed quote attack",
			clientName: `';alert(1);//`,
			wantErr:    true,
			errContain: "special characters",
		},
