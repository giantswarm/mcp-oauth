...
// validateClientStateParameter validates the client-provided state parameter.
// The client state is sent by the OAuth client in the authorization request
// for CSRF protection. When AllowNoStateParameter is enabled, empty and short
// state values are accepted for compatibility with clients that don't fully
// support the state parameter (e.g., some MCP clients).
//
...
// validateProviderStateParameter validates the server-generated provider state
// returned in the OAuth callback. Unlike client state, provider state is always
// generated by the server via generateRandomToken() and must always meet the
// minimum length requirement regardless of AllowNoStateParameter, since that
// flag only controls client-facing state validation.
//
...
func (s *Server) validateProviderStateParameter(state string) error {
	if state == "" {
