...
Different protected resources on the same domain can advertise different authorization requirements:
...
**Path Matching:** Uses longest-prefix matching. For `/mcp/files/admin/users` with paths `/mcp`, `/mcp/files`, `/mcp/files/admin` configured, the longest match (`/mcp/files/admin`) is used.

See [Discovery Mechanisms Documentation](./discovery.md#sub-path-metadata-discovery-mcp-2025-11-25) for complete details.
...
&server.Config{
    EnableClientIDMetadataDocuments: true,
    ClientMetadataCacheTTL:    5 * time.Minute,   // Cache for 5 minutes
    ClientMetadataFetchTimeout: 10 * time.Second, // 10 second timeout
}
```
