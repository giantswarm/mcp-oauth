...
- **Flexible Discovery**: Servers can now choose between WWW-Authenticate header OR well-known URI paths (June required WWW-Authenticate)
- **Sub-Path Discovery**: New per-path metadata endpoints (`/.well-known/oauth-protected-resource/<path>`)
- **Fallback Mechanism**: Clients must support both discovery methods and fall back to well-known paths if header not present
...
- **Status**: **OPTIONAL** (one of two discovery mechanisms in MCP 2025-11-25)
...
- Required WWW-Authenticate header for metadata discovery
- Basic authorization server discovery
- OAuth 2.0/2.1 baseline support

### MCP 2025-11-25 (November) - **Current**
- **Enhanced**: Flexible discovery - WWW-Authenticate OR well-known paths (both optional, servers choose one)
- **New**: Sub-path metadata discovery (`/.well-known/oauth-protected-resource/<path>`)
