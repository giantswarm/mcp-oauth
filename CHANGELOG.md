...
- `TokenRefreshHandler` no longer fires from the token-validation path when the token store does not implement `storage.TokenMetadataGetter`. Such a store cannot attribute the event, and the handler previously received it with an empty `userID` and `familyID`. `WithTokenRefreshHandler` logs a startup warning for that case.
...
- `TokenRefreshHandler` no longer fires with an empty `userID` or `familyID`. On a token-metadata miss the validation path drops the event with a debug log instead of delivering one the consumer cannot attribute. `Server.RefreshSessionProvider` now fails the call when the family record carries no user ID, instead of refreshing an entry it cannot key.
