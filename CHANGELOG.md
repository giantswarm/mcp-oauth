...
- `server.Config.ActorDelegationPolicy map[string][]string`: authorization gate for RFC 8693 delegation. Keys are actor subject patterns (matched exactly or by glob), values are the subject patterns that actor may represent. Consulted on both the client-authenticated and workload-authenticated paths when an `actor_token` is present. When nil (the default), all delegation requests are rejected — the policy must be explicitly configured before any delegated exchange will succeed.
