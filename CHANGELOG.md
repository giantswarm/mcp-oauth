...
- `server.WorkloadGrant.Groups []string`: groups added to a token minted on the workload (no-actor) exchange path, so a groupless workload such as a Kubernetes ServiceAccount token can be authorized by downstreams that gate on groups. Carried to the Exchanger as `ExchangerRequest.GrantedGroups` (the validated `Subject.Claims` are never mutated) and merged with any token-carried groups. Ignored on the delegation path, where the human subject carries its own groups. A grant that sets `Groups` must name an explicit `Issuer` and `Subject` (no `*`, no glob); `Config.Validate` rejects a wildcard group grant.
