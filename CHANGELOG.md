...
- The refresh-token family set now uses the same atomic, extend-only TTL as the user+client set, so a rotation issued under a shorter refresh-token TTL can no longer shrink it below a still-live member, and a failed TTL call can no longer leave it unbounded. A shrunk set dropped members from family-wide revocation, so `RevokeRefreshTokenFamily` could not mark the family revoked and every replay re-ran the full reuse response, `RevokeAllTokensForUserClient` included.
...
- The refresh-token family set is bounded with the same atomic, extend-only TTL as the user+client set, so a rotation can no longer shrink it below a still-live member. A shortened set dropped members from family-wide revocation, which left `RevokeRefreshTokenFamily` unable to mark the family revoked and re-ran the full reuse response, `RevokeAllTokensForUserClient` included, on every replay. The set is also no longer deleted outright when a token is saved with a non-positive TTL, and no longer left unbounded when the TTL call fails after the add.
- `RevokeRefreshTokenFamily` holds the family set, hashed and legacy key both, for `RevokedFamilyRetentionDays`, the window it already applies to each member's family metadata. The set expired first, so `GetRefreshTokenFamilyByID` stopped returning the `Revoked=true` record, `GetActiveRefreshTokenByFamily` degraded from `ErrRefreshTokenFamilyRevoked` to `ErrRefreshTokenFamilyNotFound`, and the JWT family-revocation check read that not-found as legitimate absence and admitted access tokens of a revoked family.
- The refresh-token family set is bounded with the same atomic, extend-only TTL as the user+client set. It used a separate `SADD` plus a plain `EXPIRE`, which reset the set TTL to the newly added token's horizon, so a rotation issued under a shortened `RefreshTokenTTL` could shrink the set below a still-live member's retained family metadata. Reuse detection then still fired on the rotated-away token while `RevokeRefreshTokenFamily` walked an empty set, so the family could never be marked revoked and every replay re-ran the full reuse response, including `RevokeAllTokensForUserClient`. A failed `EXPIRE` after a successful `SADD` also no longer leaves the set unbounded.
- `RevokeRefreshTokenFamily` holds the family set for `RevokedFamilyRetentionDays`, the same window it already applies to each member's family metadata (the legacy set is extended too, since both back the same index). The set kept only the residual horizon of the last add, so the revoked-family signal decayed to not-found while the metadata was still retained: `GetRefreshTokenFamilyByID` stopped returning the `Revoked=true` record, `GetActiveRefreshTokenByFamily` degraded from `ErrRefreshTokenFamilyRevoked` to `ErrRefreshTokenFamilyNotFound`, and the JWT family-revocation check read not-found as legitimate absence and admitted access tokens of a revoked family.
