...
- `server.Config.WorkloadAudiences []server.WorkloadGrant`: per-workload audience allowlist for the workload-authenticated token-exchange path. Each `WorkloadGrant` binds an issuer (matched exactly; empty matches any trusted issuer) and a subject glob to a list of permitted audience values. Only consulted when `EnableWorkloadTokenExchange` is true and an `Exchanger` is configured.
...
- `server.Config.ActorDelegationPolicy []server.DelegationGrant`: authorization gate for RFC 8693 delegation. Each `DelegationGrant` identifies an actor by issuer+subject glob and the subject issuer+subject globs that actor may represent. Issuers are matched exactly; empty matches any trusted issuer. Consulted on both the client-authenticated and workload-authenticated paths when an `actor_token` is present. When nil (the default), all delegation requests are rejected.
