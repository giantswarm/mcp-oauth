...
- **`Server.ExchangeSubjectToken` accepts an optional `ExchangeOptions`** (`Email`, `EmailVerified`, `Name`, `Groups`, `Extra map[string]any`) emitted as the standard `email`, `email_verified`, `name`, `groups` JWT claims plus any additional top-level claims supplied via `Extra`. Existing call sites compile unchanged (variadic). Library API only; the HTTP `/oauth/token` handler does not advertise these.
