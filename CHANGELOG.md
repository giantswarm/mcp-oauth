...
- **BREAKING — `server.Server` fields unexported; `Config()` getter removed; `handler.New` gains a `*server.Config` argument.** `Auditor`, `RateLimiter`, `UserRateLimiter`, `SecurityEventRateLimiter`, `ClientRegistrationRateLimiter`, `Instrumentation`, `Logger`, and `Config` are now unexported. Pass values at construction via the existing `With*` options (`server.WithAuditor`, `server.WithRateLimiter`, `server.WithUserRateLimiter`, `server.WithSecurityEventRateLimiter`, `server.WithClientRegistrationRateLimiter`, `server.WithInstrumentation`, `server.WithLogger`). Read-only access after construction is available via getters `Auditor()`, `RateLimiter()`, `UserRateLimiter()`, `SecurityEventRateLimiter()`, `ClientRegistrationRateLimiter()`, `Instrumentation()`, `Logger()`. `Config()` is not exposed; pass the same `*server.Config` pointer used in `server.New` as the second argument to `handler.New(srv, cfg, logger)` — no copying required. Closes #380.
