...
- A token the broker already minted with a delegation chain (`act`), presented to the workload exchange without a fresh `actor_token`, is now re-bound to the requested audience instead of being rejected. The new audience is authorized against the acting principal recorded in `act` (via `WorkloadAudiences`), and the existing `act` chain is preserved on the minted token (no fresh actor is nested, no workload group/subject injection is applied). The previous hard rejection (`self_minted_delegated_token_impersonation`) is removed; the security boundary it enforced is kept, because the human subject can only reach audiences the acting principal is granted. This enables a consumer to re-bind a front-door on-behalf-of token from the broker's own audience to a backend audience.
...
- `server.Config.WorkloadAudiences []server.WorkloadGrant`: per-workload audience allowlist for the workload-authenticated token-exchange path. Each `WorkloadGrant` binds an issuer (matched exactly; use `"*"` to match any trusted issuer) and a subject glob to a list of permitted audience values. An empty `Issuer` is rejected at startup by `Config.Validate`. Only consulted when `EnableWorkloadTokenExchange` is true and an `Exchanger` is configured.
...
- `server.Config.ActorDelegationPolicy []server.DelegationGrant`: authorization gate for RFC 8693 delegation. Each `DelegationGrant` identifies an actor by issuer+subject glob and the subject issuer+subject globs that actor may represent. Issuers are matched exactly; use `"*"` to match any trusted issuer. Empty issuer fields are rejected at startup by `Config.Validate`. Consulted on both the client-authenticated and workload-authenticated paths when an `actor_token` is present. When nil (the default), all delegation requests are rejected.
...
- **`Server.ExchangeSubjectToken` minted an identity-stripped token, dropping the subject's `email`, `name` and `groups`.** The local exchange path issued the access token from the caller-supplied `ExchangeOptions` only, so a delegated or `delegateToSelf` exchange (which passes no options) produced a token with no `email` or `groups` regardless of the validated subject. Downstream resource servers that key on those claims (email-to-org mapping, group-based authorization) received an anonymous-looking identity. `ExchangeSubjectToken` now defaults `email`, `email_verified`, `name` and `groups` from the validated subject's claims; an explicit `ExchangeOptions` value still takes precedence. This matches the claim propagation `LocalMintExchanger` already performs.
