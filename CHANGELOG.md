...
- **`t.Context()` in `server/introspect_test.go` and `server/refresh_session_test.go`** — replaces `context.Background()` at every call site (19 occurrences) so the test's context is cancelled at `t.Cleanup` time and any spawned goroutines unwind with the test. Closes (partial) #311.
- **`time.Sleep` removed from 4 test sites.** `storage/memory/memory_test.go` cleanup tests poll via `require.Eventually` / `require.Never` against a 20ms cleanup interval, replacing 4× 200ms fixed waits. `server/cimd_test.go` `TestMetadataCache` drives expiry through the existing `Clock` seam (`newClientMetadataCacheWithClock` + `mockClock.Advance`) instead of 2× 10ms sleeps. `storage/valkey/store_test.go` swaps two 3s post-TTL probe-sleeps for direct `TTL` queries (`-1` for no-expiry, positive for set-TTL) — same invariant, 6s shaved off the integration suite. `providers/oidc/discovery_test.go` and `exchange_test.go` `context cancellation` subtests now block on a test-controlled release channel instead of a 1s server sleep. Closes (partial) #311.
